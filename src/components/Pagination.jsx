...
import chevronLeft from "/images/left-chevron_8563242.png";
import chevronRight from "/images/right-chevron_8563331.png";
...
const Pagination = ({ count, items, page, setPage }) => {
  ...
  const previousPage = () => {
    if (page > 1 && page < lastFirstNum) {
      ...
      page === lastFirstNum ||
      (page > lastFirstNum && page < lastEndNum) ||
      page === lastEndNum
    ) {
      setPage(page - 1);
      setLastFirstNum(lastFirstNum - 1);
    ...
    <div className="boxPagination">
      <div className="wrapper">
        ...
        {pageActive.map((pageAct) => {
          ...
          return (
            <Fragment key={uuidv4()}>
              {pageAct >= firstNum && pageAct <= threePg && (
                <Button
                  buttonText={pageAct}
                  key={pageAct}
                  handleClick={() => {
              ...
              {pageAct >= lastFirstNum && pageAct <= lastEndNum && (
                <Button
                  buttonText={pageAct}
                  key={pageAct}
                  handleClick={() => {
              ...
              {TotalPg > 6 && (
                <>
                  {pageAct > threePg && pageAct < fivePg && (
                    <div key={pageAct} className="boxPageDisplay">
                      ... <div>{page}</div> ...
                    </div>
                  )}
                </>
