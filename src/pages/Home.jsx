...
const Home = ({ count, setCount, search, page, setPage, items, setShowSearch, platforms, setPlatforms, platformsArr, setPlatformsArr, genresArr, setGenresArr, genres, setGenres, noImg, setSelectOrdering }) => {
  ...
  //Sort
  const ordering = ["name", "released", "added", "created", "updated", "rating", "metacritic", "-name", "-released", "-added", "-created", "-updated", "-rating", "-metacritic"];
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/games?&page=${page}&dates=${dates}&search=${search}&search_exact=${search_exact}`)
        if (response) {
            ...
            try {
              ...
              data.forEach(datas => {
                datas.platforms.forEach(platform => {
                  ...
                  newPlatforms.push(platform.platform.name + ' ' + platform.platform.id)
                });
              });
      ...
      <div className="wrapper">
              ...
              <form className="boxFilters">
                <div className="boxFiltersPlatformType">
                  <div className="filterPlatform">
                    <Select htmlFor="Platform" labelTxt="Platform:" stateMap={platformsArr} state={setPlatforms} txtDefault="All" />
                  </div>
                  <div className="filterType">
                    <Select htmlFor="Type" labelTxt="Type:" stateMap={genresArr} state={setGenres} txtDefault="All" />
                  </div>
                </div>
                <div className="boxSortSubmit">
                  <div className="filterSort">
                    <Select htmlFor="Sort" labelTxt="Sort:" stateMap={ordering} state={setSelectOrdering} txtDefault="Default" />
                  </div>
        ...
        <div className="boxCards">
          {data.map((games, key = data.id) => {
            ...
            return (
              <Link to={`/${id}`} key={key}>
                {games.background_image ? (<Image src={games.background_image} alt={games.name} />) : games.background_image_additional ? (<Image src={games.background_image_additional} alt={data.name} />) : (<Image src={noImg} alt='no image' />)}
                <div>
                  <h3>{games.name}</h3>
                </div>
              </Link>
